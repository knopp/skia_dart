...
#include "wrapper/include/gr_context.h"
...
bool gr_context_is_supported(void) {
  return SK_ONLY_GPU(true, false);
}

void gr_direct_context_ref(gr_direct_context_t *context) {
  SK_ONLY_GPU(SkSafeRef(AsGrDirectContext(context)));
}
...
gr_direct_context_t *gr_direct_context_make_metal(void *device, void *queue) {
  ...
  context.fDevice.retain(device);
  context.fQueue.retain(queue);
  return ToGrDirectContext(GrDirectContexts::MakeMetal(context).release());
#else
  return nullptr;
#endif
}
...
gr_direct_context_t *gr_direct_context_make_metal_with_options(void *device, void *queue, const gr_context_options_t *options) {
  ...
  context.fDevice.retain(device);
  context.fQueue.retain(queue);
  return ToGrDirectContext(GrDirectContexts::MakeMetal(context, opts).release());
#else
  return nullptr;
#endif
}
...
gr_direct_context_t *gr_direct_context_make_direct3d(const gr_d3d_backendcontext_t d3dBackendContext) {
  return SK_ONLY_DIRECT3D(ToGrDirectContext(GrDirectContext::MakeDirect3D(AsGrD3DBackendContext(d3dBackendContext)).release()), nullptr);
}
...
gr_direct_context_t *gr_direct_context_make_direct3d_with_options(const gr_d3d_backendcontext_t d3dBackendContext, const gr_context_options_t *options) {
  ...
  return SK_ONLY_DIRECT3D(ToGrDirectContext(GrDirectContext::MakeDirect3D(AsGrD3DBackendContext(d3dBackendContext), opts).release()), nullptr);
}
...
bool gr_direct_context_submit(gr_direct_context_t *context, const gr_submit_info_t *submitInfo) {
  const GrSubmitInfo *info = AsGrSubmitInfo(submitInfo);
  return SK_ONLY_GPU(AsGrDirectContext(context)->submit(*info), false);
}
...
void gr_vk_extensions_init(gr_vk_extensions_t *extensions, gr_vk_get_proc getProc, void *userData, vk_instance_t *instance, vk_physical_device_t *physDev, uint32_t instanceExtensionCount, const char **instanceExtensions, uint32_t deviceExtensionCount, const char **deviceExtensions) {
  SK_ONLY_VULKAN(AsGrVkExtensions(extensions)->init([userData, getProc](const char *name, VkInstance instance, VkDevice device) -> PFN_vkVoidFunction { return getProc(userData, name, ToVkInstance(instance), ToVkDevice(device)); }, AsVkInstance(instance), AsVkPhysicalDevice(physDev), instanceExtensionCount, instanceExtensions, deviceExtensionCount, deviceExtensions));
}
