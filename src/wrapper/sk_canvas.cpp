...
void sk_canvas_draw_points(sk_canvas_t *ccanvas, sk_point_mode_t pointMode, const sk_point_t points[], size_t count, const sk_paint_t *cpaint) {
  AsCanvas(ccanvas)->drawPoints((SkCanvas::PointMode)pointMode, {AsPoint(points), count}, *AsPaint(cpaint));
}
...
template <typename T>
static SkSpan<T> SafeSpan(T *t, size_t count) {
  return t != nullptr ? SkSpan<T>{t, count} : SkSpan<T>{(T *)nullptr, 0};
}

void sk_canvas_draw_atlas(sk_canvas_t *ccanvas, const sk_image_t *atlas, const sk_rsxform_t *xform, const sk_rect_t *tex, const sk_color_t *colors, size_t count, sk_blendmode_t mode, const sk_sampling_options_t *sampling, const sk_rect_t *cullRect, const sk_paint_t *paint) {
  AsCanvas(ccanvas)->drawAtlas(AsImage(atlas), SafeSpan(AsRSXform(xform), count), SafeSpan(AsRect(tex), count), SafeSpan(colors, count), (SkBlendMode)mode, *AsSamplingOptions(sampling), AsRect(cullRect), AsPaint(paint));
}
...
gr_recording_context_t *sk_canvas_get_recording_context(sk_canvas_t *canvas) {
  return SK_ONLY_GPU(ToGrRecordingContext(SkSafeRef(AsCanvas(canvas)->recordingContext())), nullptr);
}

sk_surface_t *sk_canvas_get_surface(sk_canvas_t *canvas) {
  return ToSurface(SkSafeRef(AsCanvas(canvas)->getSurface()));
}
