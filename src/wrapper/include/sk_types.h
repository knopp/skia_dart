...
typedef struct {
  // Double to align with Flutter 64bit Matrix
  double values[9];
} sk_matrix_t;
...
/**
    A sk_path_builder_t is a helper object for creating sk_path_t
    instances by incrementally adding path segments.
*/
typedef struct sk_path_builder_t sk_path_builder_t;
...
typedef struct sk_localized_string_t sk_localized_string_t;
typedef struct sk_localized_strings_t sk_localized_strings_t;
...
typedef enum {
  SMALL_SK_PATH_BUILDER_ARC_SIZE,
  LARGE_SK_PATH_BUILDER_ARC_SIZE,
} sk_path_builder_arc_size_t;
...
typedef struct {
  ...
  int fZLibLevel;
  // TODO(knopp): Add fComments, fHdrMetadata, fGainmap and fGainmapInfo.
...
typedef struct skgpu_graphite_context_t skgpu_graphite_context_t;
typedef struct skgpu_graphite_recorder_t skgpu_graphite_recorder_t;
...
typedef struct {
  skgpu_graphite_recording_t* fRecording;
  sk_surface_t* fTargetSurface;
} skgpu_graphite_insert_recording_info_t;

typedef enum {
  SK_IMAGE_RESCALE_GAMMA_SRC,
  SK_IMAGE_RESCALE_GAMMA_LINEAR,
} sk_image_rescale_gamma_t;

typedef enum {
  SK_IMAGE_RESCALE_MODE_NEAREST,
  SK_IMAGE_RESCALE_MODE_LINEAR,
  SK_IMAGE_RESCALE_MODE_REPEATED_LINEAR,
  SK_IMAGE_RESCALE_MODE_REPEATED_CUBIC,
