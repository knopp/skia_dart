...
SK_C_API sk_image_t* sk_image_new_raster_from_compressed_texture_data(const sk_data_t* cdata, int width, int height, sk_texture_compression_type_t type);
...
SK_C_API bool sk_image_is_opaque(const sk_image_t* image);
...
SK_C_API size_t sk_image_texture_size(const sk_image_t* image);
SK_C_API bool sk_image_is_lazy_generated(const sk_image_t* image);
SK_C_API bool sk_image_is_valid(const sk_image_t* image, sk_recorder_t* recorder);
...
SK_C_API const sk_data_t* sk_image_ref_encoded(const sk_image_t* cimage);
SK_C_API sk_image_t* sk_image_make_subset_raster(const sk_image_t* cimage, const sk_irect_t* subset);
SK_C_API sk_image_t* sk_image_make_subset(const sk_image_t* cimage, sk_recorder_t* recorder, const sk_irect_t* subset);
...
SK_C_API bool sk_image_has_mipmaps(const sk_image_t* cimage);
SK_C_API bool sk_image_is_protected(const sk_image_t* cimage);
SK_C_API sk_image_t* sk_image_with_default_mipmaps(const sk_image_t* cimage);
SK_C_API sk_image_t* sk_image_reinterpret_color_space(const sk_image_t* cimage, const sk_colorspace_t* colorSpace);
SK_C_API sk_image_t* sk_image_make_color_space(const sk_image_t* cimage, sk_recorder_t* recorder, const sk_colorspace_t* colorSpace, bool mipmapped);
SK_C_API sk_image_t* sk_image_make_color_type_and_color_space(const sk_image_t* cimage, sk_recorder_t* recorder, sk_colortype_t colorType, const sk_colorspace_t* colorSpace, bool mipmapped);
SK_C_API sk_image_t* sk_image_make_scaled(const sk_image_t* cimage, sk_recorder_t* recorder, const sk_imageinfo_t* info, const sk_sampling_options_t* sampling, const sk_surfaceprops_t* props);
