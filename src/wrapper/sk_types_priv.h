...
#define DEF_CLASS_MAP(SkType, sk_type, Name) DEF_MAP_DECL(SkType, sk_type, Name, class SkType, )

#define DEF_CLASS_MAP_WITH_NS(Ns, SkType, sk_type, Name) DEF_MAP_DECL(SkType, sk_type, Name, class SkType, Ns)

#define DEF_STRUCT_MAP(SkType, sk_type, Name) DEF_MAP_DECL(SkType, sk_type, Name, struct SkType, )

#define DEF_MAP(SkType, sk_type, Name) DEF_MAP_DECL(SkType, sk_type, Name, , )

#define DEF_MAP_WITH_NS(Ns, SkType, sk_type, Name) DEF_MAP_DECL(SkType, sk_type, Name, , Ns)
...
DEF_CLASS_MAP(GrSubmitInfo, gr_submit_info_t, GrSubmitInfo);
...
DEF_MAP(SkTypeface::LocalizedStrings, sk_localized_strings_t, LocalizedStrings)
...
DEF_MAP(skresources::ImageDecodeStrategy, sk_imagedecodingstrategy_t, ImageDecodingStrategy)

static inline SkCanvas::SaveLayerRec AsCanvasSaveLayerRec(const sk_canvas_savelayerrec_t* rec) {
  return SkCanvas::SaveLayerRec(AsRect(rec->fBounds), AsPaint(rec->fPaint), AsImageFilter(rec->fBackdrop), (SkCanvas::SaveLayerFlags)rec->fFlags);
}
...
static inline SkJpegEncoder::Options AsJpegEncoderOptions(const sk_jpegencoder_options_t* options) {
  ...
  opts.fOrigin = options->fOrigin != 0 ? std::optional<SkEncodedOrigin>((SkEncodedOrigin)options->fOrigin) : std::nullopt;
  return opts;
}
...
static inline SkMatrix AsMatrix(const sk_matrix_t* matrix) {
  auto v = matrix->values;
  // SkMatrix is row-major, Dart Matrix3 is column major.
  // clang-format off
...
DEF_MAP(SkImage::RescaleGamma, sk_image_rescale_gamma_t, ImageRescaleGamma);
...
static inline GrVkBackendContext AsGrVkBackendContext(const gr_vk_backendcontext_t* context) {
  ...
  if (context->fGetProc != nullptr) {
    ctx.fGetProc = [context](const char* name, VkInstance instance, VkDevice device) -> PFN_vkVoidFunction { return context->fGetProc(context->fGetProcUserData, name, ToVkInstance(instance), ToVkDevice(device)); };
  }
...
DEF_CLASS_MAP_WITH_NS(skgpu::graphite, Context, skgpu_graphite_context_t, GraphiteContext)
DEF_CLASS_MAP_WITH_NS(skgpu::graphite, Recorder, skgpu_graphite_recorder_t, GraphiteRecorder)
DEF_CLASS_MAP_WITH_NS(skgpu::graphite, Recording, skgpu_graphite_recording_t, GraphiteRecording)
DEF_CLASS_MAP_WITH_NS(skgpu::graphite, SubmitInfo, skgpu_graphite_submit_info_t, GraphiteSubmitInfo);
DEF_CLASS_MAP_WITH_NS(skgpu::graphite, BackendTexture, skgpu_graphite_backend_texture_t, GraphiteBackendTexture);

static inline skgpu::graphite::InsertRecordingInfo AsGraphiteInsertRecordingInfo(const skgpu_graphite_insert_recording_info_t* info) {
  skgpu::graphite::InsertRecordingInfo insertInfo;
  insertInfo.fRecording = AsGraphiteRecording(info->fRecording);
  insertInfo.fTargetSurface = AsSurface(info->fTargetSurface);
  return insertInfo;
}
